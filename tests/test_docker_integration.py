...
class TestDockerContainer(unittest.TestCase):
    ...
    def setUpClass(cls):
        # Docker-Image bauen
        subprocess.run(
            ["docker", "build", "-t", "asam-ods-exd-api-xlsx", "."], check=True)
        ...
        cp = subprocess.run(
            ["docker", "run", "-d", "--rm", "--name", "test_container",
             "-p", "50051:50051", "-v", f"{data_folder}:/data", "asam-ods-exd-api-xlsx"],
    ...
    def test_structure(self):
        with grpc.insecure_channel("localhost:50051") as channel:
            ...
            handle = service.Open(oed.Identifier(
                url="/data/example_data_with_unit.xlsx",
                parameters=""), None)
            try:
                ...
                self.assertEqual(
                    structure.groups[0].channels[0].unit_string, "s")
