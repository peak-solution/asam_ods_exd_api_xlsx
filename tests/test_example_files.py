...
class TestExampleFiles(unittest.TestCase):
    ...
    def test_files(self):
        ...
        example_files_folder = pathlib.Path.joinpath(pathlib.Path(__file__).parent.resolve(), "..", "data")
        example_files = [y for x in os.walk(example_files_folder) for y in glob(os.path.join(x[0], "*.xlsx"))]
                    ...
                    for group in structure.groups:
                        ...
                        for values_channel_index, values_channel in enumerate(values.channels):
                            ...
                            if ods.DataTypeEnum.DT_COMPLEX == values_channel.values.data_type:
                                ...
                                raise ValueError(f"Unknown type {values_channel.values.data_type}")
